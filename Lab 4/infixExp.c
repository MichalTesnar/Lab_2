 ...
 * date: 13.03.2022
 *
 * description: The program builds up an expression tree from list
 * tokens according to the following grammar.
 * <expression> ::= <term> { '+'  <term> | '-' <term> }
 * <term>       ::= <factor> { '*' <factor> | '/' <factor> }
 * <factor>     ::= <number> | <identifier> | '(' <expression> ')'
 * If it is in a valid infix form, tree is created.
 * If the tree has only numerical values, then it is evaluated.
 * If it contains variables, it is first simplified and then
...
//check whether the next token is an operator without moving to the next token
...
//creates a tree structure for an expression
